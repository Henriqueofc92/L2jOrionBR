		...
		if (Config.POWERPAK_ENABLED)
		{
			...
			if (Config.L2UNLIMITED_CUSTOM)
			{
				if (DungeonManager.DG_EVENT_ENABLED)
				{
					...
					VoicedCommandHandler.getInstance().registerVoicedCommandHandler(dungeonManager);
				}
				PartyZone partyZone = new PartyZone();
				CustomBypassHandler.getInstance().registerCustomBypassHandler(partyZone);
				VoicedCommandHandler.getInstance().registerVoicedCommandHandler(partyZone);
			}
			
			{
				AutoFarm handler = new l2jorion.game.handler.voice.AutoFarm();
				VoicedCommandHandler.getInstance().registerVoicedCommandHandler(handler);
				CustomBypassHandler.getInstance().registerCustomBypassHandler(handler);
				LOG.info("Command: .autofarm - Enabled");
			}
