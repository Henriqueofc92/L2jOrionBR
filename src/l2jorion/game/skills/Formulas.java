...
public final class Formulas
{
	...
	public static boolean calcBlowSuccess(L2Character attacker, L2Character target, L2Skill skill)
	{
		int chance = (int) (skill.getPower() * 100);
		chance *= BaseStats.DEX.calcBonus(attacker);
		if (isBehind(target, attacker))
		{
			chance *= 2.0;
		}
		else if (!isInFrontOf(target, attacker))
		{
			chance *= 1.5;
		}
		if (isInFrontOf(target, attacker) && (skill.getCondition() & L2Skill.COND_BEHIND) != 0)
		{
			return false;
		}
		chance = (int) attacker.calcStat(Stats.BLOW_RATE, chance, target, null);
		if (chance > 800)
		{
			chance = 800;
		}
		
		return Rnd.get(1000) < chance;
	}
	...
	public final static double calcMagicDam(L2Character attacker, L2Character target, L2Skill skill, boolean ss, boolean bss, boolean mcrit)
	{
		...
		if (Config.EXPLLOSIVE_CUSTOM && isPvE)
		{
		...
		// Standard Magic Damage Formula
		double damage = 91 * Math.sqrt(mAtk) / mDef * skill.getPower(attacker) * calcSkillVulnerability(target, skill, skill.getSkillType());
		
		if (!target.isRaid() && !target.isRaidMinion() && target instanceof L2NpcInstance)
		{
			int lvlDiff = target.getLevel() - attacker.getLevel();
			if (lvlDiff >= 2)
			{
				double lvlPen = 1.0;
				if (lvlDiff >= 15)
		...
		// Random Damage Variance (+/- 3%)
		damage *= 1 + Rnd.get(-3, 3) / 100.0;
