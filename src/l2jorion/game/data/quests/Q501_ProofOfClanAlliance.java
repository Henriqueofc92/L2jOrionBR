...
public class Q501_ProofOfClanAlliance extends Quest
{
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		else if (event.equals("30758-03.htm"))
		{
			if (!_isSpawned && (player.getAdena() >= 10000))
			{
				...
				for (int[] coords : CHESTS_SPAWN)
					st.getQuest().addSpawn(CHESTS_ID.get(Rnd.get(CHESTS_ID.size())), coords[0], coords[1], coords[2], 0, false, 0,false);
		...
		else if (event.equals("poison"))
		{
			if (player.getAbnormalEffect() != 514)
			{
				...
				cancelQuestTimer("poison", npc, player);
			}
		...
		switch (st.getStateByte())
		{
			...
			case STATE_STARTED:
				...
				switch (npc.getNpcId())
				{
					...
					case ATHREA:
						if ((cl != null) && (cl.getInt("state") == 3) && cl.hasQuestItems(ANTIDOTE_RECIPE_LIST) && !cl.hasQuestItems(BLOOD_OF_EVA) && hasFirstHerb(st, (String) cl.get("herbs")) && (getHerbs((String) cl.get("herbs")).size() == 3))
	...
	public String onKill(L2NpcInstance npc, L2PcInstance player, boolean isPet)
	{
		...
		if (DROP.containsKey(npcId) && (cl.getInt("state") == 3))
		{
			...
			List<Integer> herbs = getHerbs((String) cl.get("herbs"));
			...
			if ((Rnd.get(10) == 1) && !st.hasQuestItems(itemId) && !hasOtherItems(st, itemId) && !herbs.contains(itemId))
			{
				if (herbs.isEmpty())
					...
					cl.set("herbs", (String) cl.get("herbs") + ";" + itemId);
	...
	public void castSkill(L2NpcInstance npc, L2PcInstance player, int skillId)
	{
