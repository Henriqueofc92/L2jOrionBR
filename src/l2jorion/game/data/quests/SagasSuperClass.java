...
public class SagasSuperClass extends Quest
{
	...
	private final Map<L2NpcInstance, Integer> _SpawnList = new HashMap<>();
	...
	private static void cast(L2NpcInstance npc, L2Character target, int skillId, int level)
	{
	...
	public void giveHallishaMark(QuestState st2)
	{
		if (st2.getInt("spawned") == 0)
		{
			if (st2.getQuestItemsCount(Items[3]) >= 700)
			{
				...
				L2NpcInstance Archon = addSpawn(Mob[1], st2.getPlayer(), false, 0, true);
				AddSpawn(st2, Archon);
				st2.set("spawned", "1");
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		if (st != null)
		{
			...
			else if (event.equalsIgnoreCase("7-1"))
			{
				...
				{
					L2NpcInstance Mob_1 = addSpawn(Mob[0], X[0], Y[0], Z[0], 0, false, 0, true);
					st.set("spawned", "1");
			...
			else if (event.equalsIgnoreCase("10-1"))
			{
				if (st.getInt("Quest0") == 0)
				{
					...
					L2NpcInstance Mob_3 = addSpawn(Mob[2], X[1], Y[1], Z[1], 0, false, 0, true);
					L2NpcInstance Mob_2 = addSpawn(NPC[4], X[2], Y[2], Z[2], 0, false, 0, true);
					AddSpawn(st, Mob_3);
					AddSpawn(st, Mob_2);
			...
			{
				// Search the NPC.
				L2NpcInstance Mob_2 = (L2NpcInstance) L2World.getInstance().findObject(st.getInt("Mob_2"));
				if (Mob_2 == null)
					return null;
