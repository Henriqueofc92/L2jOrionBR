...
public class Q384_WarehouseKeepersPastime extends Quest
{
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase(npcId + "-11.htm"))
		{
			...
			else
			{
				...
				st.set("bet", "10");
				st.set("board", scrambleString("123456789"));
			}
		}
		...
		else if (event.startsWith("select_3-")) // pick #6
		{
			...
			else
			{
				...
				final String[] board = ((String) st.get("board")).split("");
	...
	public String onKill(L2NpcInstance npc, L2PcInstance player, boolean isPet)
	{
		...
		st.dropItems(MEDAL, 1, -1, _droplist.get(npc.getNpcId()));
		
		return null;
	}
	
	private static String scrambleString(String str)
	{
	...
	{
		final String[] playerArray = ((String) st.get("playerArray")).split("");
		final String[] board = ((String) st.get("board")).split("");
