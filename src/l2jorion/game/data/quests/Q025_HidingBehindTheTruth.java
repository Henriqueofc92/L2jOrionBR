...
// import ChatType; // NOT available in L2jOrion - use int constants from Say2
...
public class Q025_HidingBehindTheTruth extends Quest
{
	...
	private static final String AMDSOUND_HORROR_02 = "AmdSound.horror_02";
		...
		switch (event)
		{
			...
			case "31533-04.html":
			{
				...
				else
				{
					qs.setMemoState(8);
					htmltext = event;
					qs.playSound(AMDSOUND_HORROR_02);
				}
				break;
			}
			case "31533-05.html":
			{
				if (qs.isMemoState(8))
				{
					if (!qs.hasQuestItems(TOTEM_DOLL3))
					{
						...
						if (step == 0)
						{
							final L2NpcInstance triyol = addSpawn(TRIOL_PAWN, TRIOL_PAWN_LOC.get(npc.getNpcId()), true, 0,false);
			...
			case "SAY_TRIYOL":
			{
				npc.broadcastPacket(new NpcSay(npc.getObjectId(), 0, npc.getNpcId(),"That box was sealed by my master."+player.getName()+" Don't touch it!"));
				break;
			}
			case "DESPAWN_TRIYOL":
			{
			...
			case "31531-02.html":
			{
				if (qs.isMemoState(11))
				{
					final L2NpcInstance box = addSpawn(COFFIN, COFFIN_LOC, true, 0,false);
