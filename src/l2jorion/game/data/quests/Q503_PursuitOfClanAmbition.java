...
public class Q503_PursuitOfClanAmbition extends Quest
{
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		else if (event.equalsIgnoreCase("30766-04.htm"))
		{
			...
			L2NpcInstance Sister1 = addSpawn(KALIS, 160665, 21209, -3710, npc.getHeading(), false, 180000, false);
			Sister1.broadcastNpcSay("Ambition and Power");
			L2NpcInstance Sister2 = addSpawn(ATHREA, 160665, 21291, -3710, npc.getHeading(), false, 180000, false);
			...
			if (element[0] == npc.getNpcId())
			{
				...
				if (st.getInt("cond") == cond)
				{
					...
					else
					{
						...
						{
							L2NpcInstance coffer = addSpawn(COFFER, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 180000, false);
	...
	private void finishQuestToClan(L2PcInstance player)
	{
		if (player.getClan() == null)
		{
			return;
		...
		for (L2PcInstance member : player.getClan().getOnlineMembers(""))
		{
			...
			QuestState qs = member.getQuestState(qn);
			if (qs != null)
			{
				qs.exitQuest(false);
			}
	...
	private void setQuestToClanMembers(L2PcInstance player)
	{
		if (player.getClan() == null)
		{
			return;
