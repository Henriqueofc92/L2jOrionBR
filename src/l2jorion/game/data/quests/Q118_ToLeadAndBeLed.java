...
public final class Q118_ToLeadAndBeLed extends Quest
{
	...
	public Q118_ToLeadAndBeLed()
	{
		...
		setItemsIds(LEG_OF_KING_ARANEID, BLOOD_OF_MAILLE_LIZARDMAN);
	}
	...
	public String onTalk(L2NpcInstance npc, L2PcInstance player)
	{
		...
		final QuestState q123 = player.getQuestState(Q123_TheLeaderAndTheFollower.class.getSimpleName());
		String htmltext = getNoQuestMsg();
		
		switch (qs.getStateByte())
		{
			case STATE_CREATED:
			{
			...
			{
				if (qs.isMemoState(1))
					htmltext = qs.getQuestItemsCount(BLOOD_OF_MAILLE_LIZARDMAN) < 10 ? "30298-04.html" : "30298-05.html";
				...
				{
					if (qs.getQuestItemsCount(LEG_OF_KING_ARANEID) < 8)
						htmltext = "30298-16.html";
					else
					{
						if (qs.getMemoStateEx(1) == 1)
						{
							qs.giveItems(CLAN_OATH_HELM, 1);
							qs.giveItems(CLAN_OATH_ARMOR, 1);
							qs.giveItems(CLAN_OATH_GAUNTLETS_HEAVY_ARMOR, 1);
							qs.giveItems(CLAN_OATH_SABATON_HEAVY_ARMOR, 1);
							qs.takeItems(LEG_OF_KING_ARANEID, -1);
						}
						else if (qs.getMemoStateEx(1) == 2)
						{
							qs.giveItems(CLAN_OATH_HELM, 1);
							qs.giveItems(CLAN_OATH_BRIGANDINE, 1);
							qs.giveItems(CLAN_OATH_LEATHER_GLOVES_LIGHT_ARMOR, 1);
							qs.giveItems(CLAN_OATH_BOOTS_LIGHT_ARMOR, 1);
							qs.takeItems(LEG_OF_KING_ARANEID, -1);
						}
						else if (qs.getMemoStateEx(1) == 3)
						{
							qs.giveItems(CLAN_OATH_HELM, 1);
							qs.giveItems(CLAN_OATH_AKETON, 1);
							qs.giveItems(CLAN_OATH_PADDED_GLOVES_ROBE, 1);
							qs.giveItems(CLAN_OATH_SANDALS_ROBE, 1);
							qs.takeItems(LEG_OF_KING_ARANEID, -1);
						}
						qs.exitQuest(false);
	...
	public String onAdvEvent(String event, L2NpcInstance npc, L2PcInstance player)
	{
		...
		if (player.getApprentice() > 0)
		{
			...
			final QuestState q118 = apprentice.getQuestState(Q118_ToLeadAndBeLed.class.getSimpleName());
			switch (event)
			{
				case "sponsor":
				{
				...
				case "30298-10.html":
				{
					if (Util.checkIfInRange(1500, npc, apprentice, true) && (q118 != null) && q118.isMemoState(2))
					{
						switch (q118.getMemoStateEx(1))
						{
							case 1:
							{
								if (q118.getQuestItemsCount(CRYSTAL_D) >= CRYSTAL_COUNT_1)
								{
									q118.takeItems(CRYSTAL_D, CRYSTAL_COUNT_1);
									q118.setMemoState(3);
									q118.set("cond", "6");
									htmltext = event;
								}
							...
							case 3:
							{
								if (q118.getQuestItemsCount(CRYSTAL_D) >= CRYSTAL_COUNT_2)
								{
									q118.takeItems(CRYSTAL_D, CRYSTAL_COUNT_2);
									q118.setMemoState(3);
									q118.set("cond", "6");
									htmltext = event;
								}
		...
		switch (event)
		{
			...
			case "30298-05d.html":
			{
				if (qs.isMemoState(1) && (qs.getQuestItemsCount(BLOOD_OF_MAILLE_LIZARDMAN) >= 10))
				{
					qs.takeItems(BLOOD_OF_MAILLE_LIZARDMAN, -1);
					qs.setMemoState(2);
