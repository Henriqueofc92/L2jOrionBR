...
public class Q335_TheSongOfTheHunter extends Quest
{
	...
	public Q335_TheSongOfTheHunter()
	{
		...
		setItemsIds(questItems.stream().mapToInt(i -> i).toArray());
	}
	...
	protected void DelItemsConds(QuestState st, int[][][] ItemsConds)
	{
		for (int[][] ItemsCond : ItemsConds)
		{
			for (int i : ItemsCond[0])
	...
	protected void GenList(QuestState st)
	{
		if ((st.get("list") == null) || ((String) st.get("list")).isEmpty())
		{
	...
	public String onAdvEvent(String event,L2NpcInstance npc, L2PcInstance player)
	{
		...
		final int state = st.getStateByte();
	...
	public String onTalk(L2NpcInstance npc, L2PcInstance player)
	{
		...
		final int _state = st.getStateByte();
	...
	public String onKill(L2NpcInstance npc, L2PcInstance player, boolean isPet)
	{
		...
		if (Items_Circle != null)
		{
			...
			if (st.getQuestItemsCount(TEST_INSTRUCTIONS1) > 0)
			{
				...
				if (npcId == BREKA_ORC_WARRIOR)
				{
					...
					{
						st.getQuest().addSpawn(BREKA_OVERLORD_HAKA, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
					else if ((Jakas_Head_count == 0) && (Rnd.get(100) < 10))
					{
						st.getQuest().addSpawn(BREKA_OVERLORD_JAKA, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
					else if ((Markas_Head_count == 0) && (Rnd.get(100) < 10))
					{
						st.getQuest().addSpawn(BREKA_OVERLORD_MARKA, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
				}
				...
				{
					st.getQuest().addSpawn(WINDSUS_ALEPH, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
				}
			}
			else if (st.getQuestItemsCount(TEST_INSTRUCTIONS2) > 0)
			{
				...
				if (npcId == TARLK_BUGBEAR_WARRIOR)
				{
					...
					{
						st.getQuest().addSpawn(TARLK_RAIDER_ATHU, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
					else if ((Lankas_Head_count == 0) && (Rnd.get(100) < 10))
					{
						st.getQuest().addSpawn(TARLK_RAIDER_LANKA, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
					else if ((Triskas_Head_count == 0) && (Rnd.get(100) < 10))
					{
						st.getQuest().addSpawn(TARLK_RAIDER_TRISKA, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
					else if ((Moturas_Head_count == 0) && (Rnd.get(100) < 10))
					{
						st.getQuest().addSpawn(TARLK_RAIDER_MOTURA, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
					else if ((Kalaths_Head_count == 0) && (Rnd.get(100) < 10))
					{
						st.getQuest().addSpawn(TARLK_RAIDER_KALATH, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
					}
				}
		...
		if ((st.getQuestItemsCount(CIRCLE_HUNTER_LICENSE1) > 0) || (st.getQuestItemsCount(CIRCLE_HUNTER_LICENSE2) > 0))
		{
			...
			if (request != null)
			{
				...
				if (request.spawnlist.containsKey(npcId) && (st.getQuestItemsCount(request.request_item) < request.request_count))
				{
					...
					{
						st.getQuest().addSpawn(spawn_n_chance[0], npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000, false);
		...
		if (((npcId == 27160) || (npcId == 27162) || (npcId == 27164)) && (Rnd.get(100) < 50))
		{
			...
			st.getQuest().addSpawn(27150, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
			st.getQuest().addSpawn(27150, npc.getX(), npc.getY(), npc.getZ(), npc.getHeading(), true, 300000,false);
		}
		return null;
	}
