...
public class KnownListUpdateTaskManager
{
	...
	public void updateRegion(L2WorldRegion region, boolean fullUpdate, boolean forgetObjects)
	{
		...
		if (region.getVisibleObjects().isEmpty())
		{
			return;
		}
		
		Collection<L2Object> vObj = new ArrayList<>(region.getVisibleObjects().values());
		...
		for (L2Object object : vObj)
		{
			...
			for (L2WorldRegion regi : region.getSurroundingRegions())
			{
				...
				{
					if (!regi.getVisibleObjects().isEmpty())
					{
						Collection<L2Object> inrObj = new ArrayList<>(regi.getVisibleObjects().values());
				...
				else if (object instanceof L2Character)
				{
					if (regi.isActive())
					{
						if (!regi.getVisiblePlayable().isEmpty())
						{
							Collection<L2PlayableInstance> inrPls = new ArrayList<>(regi.getVisiblePlayable().values());
