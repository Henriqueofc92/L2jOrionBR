...
public class AttackStanceTaskManager
{
	...
	private class FightModeScheduler implements Runnable
	{
		...
		public void run()
		{
			...
			try
			{
				if (_attackStanceTasks != null && !_attackStanceTasks.isEmpty())
				{
					for (final L2Character actor : new ArrayList<>(_attackStanceTasks.keySet()))
					{
						Long lastAttackTime = _attackStanceTasks.get(actor);
						if (lastAttackTime == null)
						{
							continue;
						}
						
						if ((current - lastAttackTime) > 15000)
						{
