		...
		else if (command.startsWith("SevenSigns"))
		{
			...
			if (command.length() > 13)
			{
				...
				catch (Exception e)
				{
					...
					catch (Exception e2)
					{
						...
						{
							LOG.warn("Failed to retrieve cabal from bypass command. NpcId: " + getNpcId() + "; Command: " + command);
						}
					}
				}
			}
			...
			switch (val)
			{
				...
				case 17: // Exchange Seal Stones for Ancient Adena (Type Choice) - SevenSigns 17 x
					...
					{
						LOG.warn("Problem with HTML text " + SevenSigns.SEVEN_SIGNS_HTML_PATH + "signs_17.htm: " + path);
					}
