...
public class L2PcInstance extends L2PlayableInstance
{
	...
	private HashMap<L2Skill, int[]> _cancelledBuffs = new HashMap<>();
	...
	public void onAction(L2PcInstance player)
	{
		...
		if (isAutoFarm())
		{
			AutofarmManager.getInstance().stopFarm(this);
			sendMessage("Auto Farm cancelado devido a movimento manual.");
		}
	...
	
	{
		...
		if (DungeonManager.getPlayers().contains(this))
		{
			DungeonManager.getInstance().onDeath(this, killer);
			unsummonAllCubics();
			if (_forceBuff != null)
			{
				abortCast();
	...
	{
		if (atEvent || (TvT.is_started() && _inEventTvT) || (DM.is_started() && _inEventDM) || (CTF.is_started() && _inEventCTF) || (VIP._started && _inEventVIP)
			|| (getInstanceId() > 0 && (getInstanceId() == PartyZone.INSTANCE_ID || getInstanceId() == DungeonManager.INSTANCE_PVP || getInstanceId() == DungeonManager.INSTANCE_SOLO)) || killer == null)
		{
			return;
		}
	...
	public boolean isAutoAttackable(L2Character attacker)
	{
		...
		if (attacker instanceof L2PcInstance)
		{
			...
			if (isInsideZone(ZoneId.ZONE_PVP) && attacker.isInsideZone(ZoneId.ZONE_PVP))
			{
				return true;
			}
	...
	public ArrayList<Buff> getOwnBuffs(int objectId)
	{
		if (_savedAllBuffs.get(objectId) == null)
		{
			...
			{
				_savedAllBuffs.put(objectId, new ArrayList<>());
			}
		}
	...
	public HashMap<L2Skill, int[]> getCancelledBuffs()
	{
	...
	private boolean _isAutoFarm = false;
	private int _autoFarmPage = 0;
	...
	private boolean _isSummonAttack = false;
	private boolean _antiKsProtected = false;
	private boolean _noBuffProtected = false;
	private boolean _isResting = false;
	private final Set<Integer> _ignoredMonsters = new LinkedHashSet<>();
	
	public AutofarmPlayerRoutine getBot()
	{
		return AutofarmManager.getInstance().getRoutine(getObjectId());
	}
	...
	public int getPage()
	{
		return _autoFarmPage;
	}
	
	public void setPage(int val)
	{
		_autoFarmPage = Math.max(0, Math.min(9, val));
	}
	...
	public void setLooting(boolean val)
	{
		_isLooting = val;
	}
	...
	public void setAutoFarmRadius(int val)
	{
		_autoFarmRadius = Math.max(200, Math.min(5000, val));
	}
	...
	{
		return _noBuffProtected;
	}
	
	public void setNoBuffProtection(boolean val)
	{
		_noBuffProtected = val;
	}
	...
	public boolean ignoredMonsterContain(int id)
	{
		return _ignoredMonsters.contains(id);
	}
	
	public void addIgnoredMonster(int id)
	{
		_ignoredMonsters.add(id);
	}
	
	public void activeMonster(int id)
	{
		_ignoredMonsters.remove(id);
	}
