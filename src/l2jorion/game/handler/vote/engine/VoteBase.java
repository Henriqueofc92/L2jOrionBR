...
public abstract class VoteBase
{
	...
	public void updateDB(L2PcInstance player, String column)
	{
		...
		try
		{
			...
			statement = con.prepareStatement(String.format("UPDATE accounts_voted set %s=? where vote_ip=?", column));
		...
		{
			LOG.error("Error in VoteBase.updateDB:", e);
		}
		finally
		{
			try
			...
			{
				LOG.error("Error closing statement in VoteBase:", e);
			}
			CloseUtil.close(con);
		}
	}
	...
	public boolean hasVoted(L2PcInstance player)
	{
		try
		{
			...
			if (endpoint == null || endpoint.startsWith("error"))
			{
				return false;
			}
		...
		catch (Exception e)
		{
			...
			LOG.error("Error checking vote status:", e);
		}
		return false;
	}
	...
	public String getApiResponse(String endpoint)
	{
		...
		try
		{
			...
			{
				LOG.error("Votebase: read error", e);
			}
		...
		{
			LOG.error(getClass().getSimpleName() + ": Error:", e);
			return "error";
		}
	}
}