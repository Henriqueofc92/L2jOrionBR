...
public abstract class IdFactory
{
	...
	private void setAllCharactersOffline()
	{
		...
		{
			LOG.error("Could not update characters online status: " + e.getMessage());
		}
	}
	...
	private void cleanUpDB()
	{
			...
			Statement stmt = con.createStatement())
		{
			...
			if (cleanCount > 0)
			{
				LOG.info("IdFactory: Cleaned " + cleanCount + " elements from database.");
			}
		}
		catch (final SQLException e)
		{
			...
			LOG.error("IdFactory: Error during DB cleanup: " + e.getMessage());
		}
	}
	...
	protected int[] extractUsedObjectIDTable() throws SQLException
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection();
			Statement s = con.createStatement())
		{
			s.executeUpdate("CREATE TEMPORARY TABLE IF NOT EXISTS temp_used_ids (object_id INT NOT NULL PRIMARY KEY) ENGINE=MEMORY");
			s.executeUpdate("TRUNCATE TABLE temp_used_ids");
			s.executeUpdate("DELETE FROM itemsonground WHERE object_id IN (SELECT object_id FROM items)");
			s.executeUpdate("INSERT IGNORE INTO temp_used_ids (object_id) SELECT obj_id FROM characters");
			s.executeUpdate("INSERT IGNORE INTO temp_used_ids (object_id) SELECT object_id FROM items");
			s.executeUpdate("INSERT IGNORE INTO temp_used_ids (object_id) SELECT clan_id FROM clan_data");
			s.executeUpdate("INSERT IGNORE INTO temp_used_ids (object_id) SELECT object_id FROM itemsonground");
			try (ResultSet result = s.executeQuery("SELECT object_id FROM temp_used_ids ORDER BY object_id"))
			{
				result.last();
