...
public class DayNightSpawnManager
{
	...
	private DayNightSpawnManager()
	{
		...
		_bosses = Collections.synchronizedMap(new HashMap<>());
	}
	...
	private void spawnCreatures(List<L2Spawn> unSpawnCreatures, List<L2Spawn> spawnCreatures, String UnspawnLogInfo, String SpawnLogInfo)
	{
		try
		{
			...
			{
				int i = 0;
				for (L2Spawn spawn : new ArrayList<>(unSpawnCreatures))
				{
					if (spawn == null)
					{
						continue;
			...
			int i = 0;
			for (L2Spawn spawnDat : new ArrayList<>(spawnCreatures))
			{
				if (spawnDat == null)
				{
					continue;
	...
	private void specialNightBoss(int mode)
	{
		try
		{
			List<L2Spawn> allSpawns;
			synchronized (_bosses)
			{
				allSpawns = new ArrayList<>(_bosses.keySet());
			}
			
			for (L2Spawn spawn : allSpawns)
			{
	...
	private void ShadowSenseMsg(int mode)
	{
		...
		final Collection<L2PcInstance> pls = new ArrayList<>(L2World.getInstance().getAllPlayers().values());
		for (L2PcInstance onlinePlayer : pls)
		{
			if (onlinePlayer != null && onlinePlayer.isOnline() == 1 && onlinePlayer.getRace().ordinal() == 2 && onlinePlayer.getSkillLevel(294) > 0)
			{
