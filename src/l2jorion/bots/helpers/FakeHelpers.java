	...
	public static FakePlayer createRandomFakePlayer(int level, int classNumber, boolean pvpZone, boolean farmer)
	{
		...
		switch (level)
		{
			case 1:
			{
				...
				{
					for (final PcTemplateItem pcItem : player.getBaseTemplate().getItems())
					{
						final L2ItemInstance item = player.getInventory().addItem("Init", pcItem.getItem().getItemId(), pcItem.getAmount(), player, null);
