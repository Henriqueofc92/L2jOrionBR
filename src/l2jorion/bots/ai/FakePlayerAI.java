...
public abstract class FakePlayerAI
{
	...
	protected void applyDefaultBuffs()
	{
		for (int[] buff : getBuffs())
		{
			...
			{
				LOG.error("Error applying default buffs for FakePlayer: " + _fakePlayer.getName(), e);
			}
		}
	}
		...
		else
		{
			...
			if (_fakePlayer.getBotMode() != 3)
			{
				if (_fakePlayer.getTarget() != null && !_fakePlayer.isMoving() && !_fakePlayer.isTeleporting()
					&& !GeoData.getInstance().canMove(_fakePlayer.getX(), _fakePlayer.getY(), _fakePlayer.getZ(), _fakePlayer.getTarget().getX(), _fakePlayer.getTarget().getY(), _fakePlayer.getTarget().getZ(), _fakePlayer.getInstanceId()))
				{
					_fakePlayer.setTarget(null);
					return;
				}
