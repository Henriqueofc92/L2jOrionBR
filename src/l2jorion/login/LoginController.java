...
public class LoginController
{
	...
	protected ArrayList<L2LoginClient> _clients = new ArrayList<>();
	
	/** Authed Clients on LoginServer */
	protected Map<String, L2LoginClient> _loginServerClients = new HashMap<>();
	
	private Map<InetAddress, BanInfo> _bannedIps = new HashMap<>();
	...
	class PurgeThread extends Thread
	{
		...
		public void run()
		{
			while (!isInterrupted())
			{
				synchronized (_clients)
				{
					for (L2LoginClient client : new ArrayList<>(_clients))
					{
				...
				synchronized (_loginServerClients)
				{
					for (L2LoginClient client : new ArrayList<>(_loginServerClients.values()))
					{
						if (client.getConnectionStartTime() + LOGIN_TIMEOUT >= System.currentTimeMillis())
						{
							client.close(LoginFailReason.REASON_ACCESS_FAILED);
